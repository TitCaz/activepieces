...
export const listSupplierInvoices = createAction({
  auth: dolibarrAuth,
  name: 'list_supplier_invoices',
  displayName: 'Lister les factures fournisseur',
  description: 'Récupère la liste des factures fournisseur dans Dolibarr.',
  props: {
  ...
  async run(context) {
    ...
    return dolibarrRequest({ auth: context.auth.props, method: HttpMethod.GET, endpoint: '/supplierinvoices', queryParams });
  },
});
