...
export const listDevisdmg = createAction({
  auth: dolibarrAuth,
  name: 'list_devisdmg',
  displayName: 'Lister les devis DMG',
  description: 'Récupère la liste des devis DMG (Redesk).',
  props: {
  ...
  async run(context) {
    ...
    return dolibarrRequest({ auth: context.auth.props, method: HttpMethod.GET, endpoint: '/devisdmgapi', queryParams });
  },
});
