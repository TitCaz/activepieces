...
export const updateContact = createAction({
  auth: dolibarrAuth,
  name: 'update_contact',
  displayName: 'Mettre à jour un contact',
  description: 'Met à jour un contact existant dans Dolibarr.',
  props: {
    id: Property.ShortText({ displayName: 'ID du contact', required: true }),
    lastname: Property.ShortText({ displayName: 'Nom', required: false }),
  ...
  async run(context) {
    const { id, lastname, firstname, email, phone_pro, phone_mobile, poste } = context.propsValue;
    ...
    return dolibarrRequest({ auth: context.auth.props, method: HttpMethod.PUT, endpoint: `/contacts/${id}`, body });
  },
});
