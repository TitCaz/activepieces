...
export const listContracts = createAction({
  auth: dolibarrAuth,
  name: 'list_contracts',
  displayName: 'Lister les contrats',
  description: 'Récupère la liste des contrats dans Dolibarr.',
  props: {
    ...
    sqlfilters: Property.ShortText({ displayName: 'Filtres SQL', description: "Filtre avancé, ex: (t.ref:like:'CON%')", required: false }),
  },
  async run(context) {
    ...
    return dolibarrRequest({ auth: context.auth.props, method: HttpMethod.GET, endpoint: '/contracts', queryParams });
  },
});
