...
export const listSetupCountries = createAction({
  auth: dolibarrAuth,
  name: 'list_setup_countries',
  displayName: 'Lister les pays (dictionnaire)',
  description: 'Récupère la liste des pays disponibles dans Dolibarr.',
  props: {
  ...
  async run(context) {
    const { limit, page, sortfield, sortorder } = context.propsValue;
    return dolibarrRequest({
      auth: context.auth.props,
      method: HttpMethod.GET,
      endpoint: '/setup/dictionary/countries',
      queryParams: { limit: String(limit ?? 50), page: String(page ?? 0), sortfield: sortfield ?? 't.rowid', sortorder: sortorder ?? 'DESC' },
    });
  },
});
