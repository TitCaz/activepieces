...
export const listUsers = createAction({
  ...
  description: 'Récupère la liste des utilisateurs dans Dolibarr.',
  props: {
  ...
  async run(context) {
    ...
    return dolibarrRequest({ auth: context.auth.props, method: HttpMethod.GET, endpoint: '/users', queryParams });
  },
});
